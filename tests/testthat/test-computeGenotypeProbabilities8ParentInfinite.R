...
test_that("Test non-zero generations of intercrossing",
	{
		...
		map <- sim.map(len = c(100, 100), n.mar = 101, anchor.tel = TRUE, include.x=FALSE, eq.spacing=TRUE)
		pedigrees <- list()
		pedigrees[[1]] <- eightParentPedigreeRandomFunnels(initialPopulationSize = 1000, selfingGenerations = 6, nSeeds = 1, intercrossingGenerations = 1)
		pedigrees[[2]] <- eightParentPedigreeRandomFunnels(initialPopulationSize = 1000, selfingGenerations = 6, nSeeds = 1, intercrossingGenerations = 2)
		pedigrees[[3]] <- eightParentPedigreeSingleFunnel(initialPopulationSize = 1000, selfingGenerations = 6, nSeeds = 1, intercrossingGenerations = 1)
		pedigrees[[4]] <- eightParentPedigreeSingleFunnel(initialPopulationSize = 1000, selfingGenerations = 6, nSeeds = 1, intercrossingGenerations = 2)
		for(pedigree in pedigrees)
		{
			pedigree@selfing <- "infinite"
			testFunc(map, pedigree)
