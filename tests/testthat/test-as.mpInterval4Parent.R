context("Conversion to mpwgaim object")
if(suppressWarnings(require(mpwgaim)) && suppressWarnings(require(mpMap)))
{
	...
	{
		pedigree <- fourParentPedigreeSingleFunnel(initialPopulationSize = 500, selfingGenerations = 10, nSeeds = 1, intercrossingGenerations = 0)
		...
		old$fid <- 1:4
		class(old) <- "mpcross"
		oldConverted <- mpcross2int(old, gen.type="mpMarker", method = "qtl")
